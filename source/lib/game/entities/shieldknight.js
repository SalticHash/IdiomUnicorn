...
ig.baked=true;ig.module('game.entities.shieldknight').requires('impact.entity','impact.entity-pool').defines(function(){EntityShieldknight=ig.Entity.extend({size:{x:25,y:20},offset:{x:5,y:4},maxVel:{x:500,y:1000},storeMaxVel:{x:500,y:1000},storeVel:{x:null,y:null},friction:{x:400,y:0},zIndex:1,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.PASSIVE,health:100,speed:0,flip:false,idle:false,pause:false,kOTB:false,waitingBro:true,synced:false,delay:1,jumpTime:.66,jumping:false,landed:true,name:"shieldKnight",attackSound:new ig.Sound('media/sounds/shield.*'),deadSound:new ig.Sound('media/sounds/shield-dead.*'),_wmDrawBox:true,_wmBoxColor:'rgb(65, 42, 9, 1)',animSheets:{sheetOne:new ig.AnimationSheet('media/shield-knight-01.png',40,32),},init:function(x,y,settings){this.parent(x,y,settings);this.jumpTimer=new ig.Timer(this.jumpTime);this.idleTimer=new ig.Timer(0);this.idlingTimer=new ig.Timer(0);this.runTimer=new ig.Timer(0);this.attackTimer=new ig.Timer(0);this.dieUpTimer=new ig.Timer(0);this.delayTimer=new ig.Timer(0);this.anims.idle=new ig.Animation(this.animSheets.sheetOne,.1,[0,1,2,3,4,5,6]);this.anims.jump=new ig.Animation(this.animSheets.sheetOne,1,[8]);this.anims.fall=new ig.Animation(this.animSheets.sheetOne,1,[9]);this.waitingBro=true;this.synced=false;},reset:function(x,y,settings){this.parent(x,y,settings);this.kOTB=false;this.waitingBro=true;this.synced=false;},pauseTimers:function(){this.jumpTimer.pause();this.runTimer.pause();this.idleTimer.pause();this.idlingTimer.pause();this.attackTimer.pause();this.dieUpTimer.pause();},unpauseTimers:function(){this.jumpTimer.unpause();this.runTimer.unpause();this.idleTimer.unpause();this.idlingTimer.unpause();this.attackTimer.unpause();this.dieUpTimer.unpause();},paused:function(){if(!this.storeVel.x){this.storeVel.x=this.vel.x;}
...
this.parent();},checkCond:function(){if(!this.jumping&&this.jumpTimer.delta()>0&&this.landed){this.landed=false;this.jumping=true;}},movements:function(){if(this.kOTB){if(ig.game.getEntityByName('player')){var player=ig.game.getEntityByName('player');if(player.pos.x>this.pos.x){this.vel.x=-220;}
else{this.vel.x=220;}}
...
this.vel.y=1000*bFF;}}
else{if(this.jumpTimer.delta()>0&&this.jumping){this.vel.y=-366;this.jumping=false;}
...
else{this.parent(res);if(res.collision.y){if(!this.waitingBro&&!this.landed){this.landed=true;this.jumpTimer.set(this.jumpTime);}}
if(res.collision.x){this.flip=!this.flip;this.offset.x=this.flip?0:24;}}},knockMeOutTheBox:function(){this.kOTB=true;this.dieUpTimer.set(.35);if(ig.game.getEntityByName('player')){var player=ig.game.getEntityByName('player')
if(!ig.game.muteGame){this.deadSound.volume=player.attacking?.085:.1;this.deadSound.play();}}
...
if(other==player){if(!ig.game.quiz&&this.attackTimer.delta()>0&&!this.kOTB){ig.game.quizbox.quiz(1,this.name);if(!ig.game.muteGame){this.attackSound.volume=.25;this.attackSound.play();}
this.receiveDamage(1,other);this.attackTimer.set(ig.game.enemyRecoveryTime);}
if(!this.kOTB){this.knockMeOutTheBox();}}}}});ig.EntityPool.enableFor(EntityShieldknight);});
